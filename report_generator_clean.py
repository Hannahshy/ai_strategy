...
| 行业 | 企业数 |
|------|--------|
{'| '.join([f"{k} | {v} |" for k, v in industry_dist.items()])}

### 企业规模分布
| 规模 | 企业数 |
|------|--------|
{'| '.join([f"{k} | {v} |" for k, v in size_dist.items()])}

### 成熟度等级分布
| 等级 | 企业数 |
|------|--------|
{'| '.join([f"{k} | {v} |" for k, v in maturity_dist.items()])}

### 行业平均成熟度排名
| 排名 | 行业 | 平均分 |
|------|------|--------|
{'| '.join([f"{i+1} | {idx} | {val:.2f} |" for i, (idx, val) in enumerate(industry_avg.items())])}

### 六大维度平均分
| 维度 | 平均分 | 状态 |
|------|--------|------|
{'| '.join([f"{dim} | {score:.2f} | {'🟢 良好' if score >= 3.5 else '🟡 一般' if score >= 2.5 else '🔴 不足'} |" for dim, score in dim_avg.items()])}
"""
    return overview
...
| 维度 | 差距值 | 严重程度 |
|------|--------|----------|
{'| '.join([f"{dim} | {gap:.2f} | {'🔴 严重' if gap >= 1.5 else '🟡 中等' if gap >= 1.0 else '🟢 轻微'} |" for dim, gap in problem_dims])}
...
| 行业 | 差距值 | 状态 |
|------|--------|------|
{'| '.join([f"{industry} | {gap:+.2f} | {'🟢 领先' if gap > 0 else '🔴 落后'} |" for industry, gap in industry_gaps.items()])}
...
|------|------------|------------|------------|
{'| '.join([f"{dim} | {row['priority_score']:.2f} | {row['impact']:.2f} | {row['effort']:.2f} |" for dim, row in initiative_stats.iterrows()])}
...
|------|----------|----------|------------|----------|----------|
{'| '.join([f"{i+1} | {rec['initiative_name']} | {rec['dimension']} | {rec['priority_score']:.2f} | {rec['duration_months']}个月 | {rec['phase']} |" for i, (_, rec) in enumerate(top_priority_actions.iterrows())])}
